...
export function MainScreen({ token, onLogout }: MainScreenProps) {
  ...
  const handleSendText = async () => {
    ...
    try {
      ...
      pushMessages([createMessage("assistant_text", response.reply)]);
    } catch (error: any) {
  ...
  const handleUploadPress = async () => {
    ...
    try {
      ...
      await uploadDocument({
        ...
        file: {
          uri,
          name: picked.name ?? `document-${Date.now()}.pdf`,
          type: picked.type ?? "application/pdf",
        },
        category: "general",
      });

      pushSystemMessage("Belge y\u00fcklendi ve \u00f6\u011frenme pipeline'\u0131na g\u00f6nderildi.");
    } catch (error: any) {
